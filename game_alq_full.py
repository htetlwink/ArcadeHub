# game_alq_full.py - Full Traditional Alquerque (Diagonals & Mandatory Captures)
import random
import time
import gc
from arcade_common import (
    ...
    draw_circle_ring, draw_thick_line_diag, draw_button, draw_header_bar,
    scoreboard
)

class AlquerqueFullGame:
    def __init__(self):
    ...
    def from_dict(self, data):
        ...
        self.count_pieces()
        if self.selected:
            jumps_exist = self.has_any_jumps_for_player(self.turn)
            moves = self.get_legal_moves_for_piece(self.selected[0], self.selected[1], force_jumps_only=jumps_exist)
    ...
    def get_node_directions(self, r, c):
        """Returns valid movement vectors (dr, dc) for node (r, c)."""
        # All nodes have 4 orthogonal connections
        dirs = [(-1, 0), (1, 0), (0, -1), (0, 1)]
        # Nodes with even parity (r + c) % 2 == 0 ALSO have 4 diagonal connections
        if (r + c) % 2 == 0:
    ...
    def get_legal_moves_for_piece(self, r, c, force_jumps_only=False):
        ...
        for dr, dc in dirs:
            ...
            # Jump capture check (over opponent into empty space)
            ...
            # Step move check (1 step to adjacent empty node along connecting line)
        ...
        if jumps or force_jumps_only or self.must_continue_jump:
            return jumps
        return steps
    ...
    def get_all_legal_moves(self, player):
        jumps_exist = self.has_any_jumps_for_player(player)
        all_moves = []
        for r in range(5):
            for c in range(5):
                if self.grid[r][c] == player:
    ...
    def select_cell(self, r, c):
        ...
        current_p = self.turn
        jumps_exist = self.has_any_jumps_for_player(current_p)

        # If already selected a piece, check if tapping a valid target node
        ...
        # If in a mandatory chain jump, piece selection cannot be changed
    ...
    def execute_move(self, from_pos, to_pos, is_jump, captured_pos):
        ...
        # Check for multi-jump (chain captures)
        further_jumps = []
        if is_jump:
        ...
        if is_jump and further_jumps:
            ...
            # Indicate if mandatory capture exists for next turn
            if self.has_any_jumps_for_player(self.turn):
                self.status_msg = f"{p_name}: JUMP MANDATORY!"
            else:
    ...
    def ai_move(self):
        ...
        if selected_move:
            ...
            while self.must_continue_jump and not self.game_over:
                sr, sc = self.selected
                chain_jumps = self.get_legal_moves_for_piece(sr, sc, force_jumps_only=True)
                if chain_jumps:
                    best_cj = None
                    best_score = -999999
...
def render_node_tile_full(tft, r, c, val, is_sel, is_target):
    cx = 30 + c * 65
    cy = 110 + r * 68

    fill_tile_buf(COLOR_CARD_BG)

    # Base grid lines inside tile
...
def init_alq_full_ui(tft, game):
    tft.fill_rect(0, 0, 320, 480, COLOR_DARK_BG)
    draw_header_bar(tft, "ALQUERQUE PRO")
    ...
    # Draw full traditional diagonal lines
    for r in range(5):
        for c in range(5):
            if (r + c) % 2 == 0:
                cx1 = 30 + c * 65
    ...
    # Force full redraw of grid and status
    for r in range(5):
        for c in range(5):
    ...
    game._prev_p1 = -1
    game._prev_p2 = -1
    update_alq_full_ui(tft, game)


def update_alq_full_ui(tft, game):
    ...
    for r in range(5):
        for c in range(5):
            ...
            if val != prev_val or is_sel != was_sel or is_target != was_target:
                render_node_tile_full(tft, r, c, val, is_sel, is_target)
    ...
    if (game.status_msg != game.prev_status_msg or
        ...
        msg_color = COLOR_GOOGLE_RED if "MANDATORY" in game.status_msg else COLOR_GOOGLE_YELLOW
        tft.draw_text(game.status_msg, 20, 432, msg_color, bg=COLOR_CARD_BG, scale=scale)
